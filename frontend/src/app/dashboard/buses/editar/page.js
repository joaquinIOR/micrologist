...
export default function EditarBus() {
  ...
  useEffect(() => {
    ...
    api.buses.listar().then(buses => {
      ...
      setForm({
        ...
        revision_tecnica:    bus.revision_tecnica    || "",
        soap:                bus.soap                || "",
        permiso_circulacion: bus.permiso_circulacion || "",
        notas:               bus.notas               || "",
      });
      setCargando(false);
    });
  ...
  const guardar = async () => {
    ...
    try {
      ...
      await api.buses.actualizar(id, payload);
  ...
  const eliminar = async () => {
    if (!confirm("¿Estás seguro que quieres eliminar este bus?")) return;
    try {
      await api.buses.eliminar(id);
      window.location.href = "/dashboard";
    } catch (err) {
      setError(err.message);
    }
      ...
      <div style={{ maxWidth: 600, margin: "0 auto" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "2rem" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            ...
            <h1 style={{ fontSize: 22, fontWeight: 700, margin: 0 }}>Editar bus</h1>
          </div>
          <button onClick={eliminar} style={{ padding: "0.5rem 1rem", background: "rgba(239,68,68,0.15)", border: "1px solid rgba(239,68,68,0.3)", borderRadius: 8, color: "#f87171", fontSize: 13, fontWeight: 600, cursor: "pointer" }}>
            🗑 Eliminar bus
          </button>
        </div>
