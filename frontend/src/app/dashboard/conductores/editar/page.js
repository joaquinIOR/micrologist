...
function EditarConductorForm() {
  ...
  const guardar = async () => {
    ...
    try {
      ...
      await api.conductores.actualizar(id, payload);
  ...
  const eliminar = async () => {
    if (!confirm("¿Estás seguro que quieres eliminar este conductor?")) return;
    try {
      await api.conductores.eliminar(id);
      window.location.href = "/dashboard";
    } catch (err) {
      setError(err.message);
    }
      ...
      <div style={{ maxWidth: 600, margin: "0 auto" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "2rem" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            ...
            <h1 style={{ fontSize: 22, fontWeight: 700, margin: 0 }}>Editar conductor</h1>
          </div>
